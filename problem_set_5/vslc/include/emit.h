...
// if cnt is a register, is must be among the "classics", such as %rcx
...
// these are mine
#define CALL(label)       EMIT("call %s", (label)) // Call a function
#define ADDR(label) "$" label
#define INSTRUCTION(fmt, ...) printf(fmt "\n", ##__VA_ARGS__)
#define IMM(value) "$" #value
#define LEAQ(src,dst)     EMIT("leaq %s, %s", (src), (dst)) // Add LEAQ definition
